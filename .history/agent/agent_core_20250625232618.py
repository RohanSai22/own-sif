...
"""
    ...
    def _research_improvements(self, weaknesses: str) -> str:
        """
        Actively research improvements using LLM and web search.
        1. Use LLM to generate search queries from weaknesses.
        2. Use web_search tool to get results for each query.
        3. Use LLM to synthesize actionable findings from search results.
        4. Consolidate and return the findings.
        """
        try:
            tui.log_thought("[Research] Generating search queries from weaknesses...")
            # Step 1: Generate search queries
            query_prompt = (
                "You are an expert AI researcher. Your goal is to improve a software engineering agent. "
                "Based on the following weaknesses: '" + weaknesses + "', generate 3-5 specific, effective search queries "
                "to find novel algorithms, coding patterns, or architectural improvements. "
                "Return ONLY a numbered list of queries."
            )
            query_response = self._generate_response([
                {"role": "user", "content": query_prompt}
            ])
            queries = []
            for line in query_response.content.splitlines():
            ...
            tui.log_thought(f"[Research] Generated queries: {queries}")

            # Step 2: Execute web search for each query
            all_summaries = []
            for query in queries:
                tui.log_thought(f"[Research] Searching: {query}")
                try:
                    search_results = self.tool_manager.execute_tool("web_search", query, max_results=3)
                except Exception as e:
                    tui.log_action("web_search", f"Failed: {e}", "ERROR")
                    continue
                # Step 3: Synthesize findings from search results
                search_text = "\n\n".join(
                    f"Title: {r.get('title','')[:80]}\nSnippet: {r.get('body','')[:300]}" for r in search_results if r)
                synth_prompt = (
                    f"Here are the search results for '{query}':\n\n{search_text}\n\n"
                    "Summarize the most promising and actionable techniques from this text that could be applied "
                    "to improve an AI agent's source code. Focus on concrete, implementable ideas."
                )
                synth_response = self._generate_response([
                    {"role": "user", "content": synth_prompt}
                ])
                summary = synth_response.content.strip()
                if summary:
                    all_summaries.append(f"Query: {query}\n{summary}")
            # Step 4: Consolidate findings
            if not all_summaries:
                return "No actionable research findings could be synthesized."
            consolidated = "\n\n".join(all_summaries)
            return consolidated
        except Exception as e:
            logger.error(f"Research improvements failed: {e}")
            return f"Research improvements failed: {e}"
