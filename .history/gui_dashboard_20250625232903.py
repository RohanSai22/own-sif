...
"""
    ...
    def start_monitoring(self):
        """Start the monitoring and update loop using live_state.json."""
        import json
        import time
        import os
        state_path = os.path.join(self.project_root, "live_state.json")
        last_mtime = None
        def update_loop():
            while True:
                try:
    ...
    def _update_from_state(self, state: dict):
        """Update all GUI panels from the shared state file."""
        # Example: update quick stats, performance charts, generation history, etc.
        self.current_iteration = state.get("iteration", 0)
        self.generation_history = state.get("generation_history", [])
        self.current_status = state.get("status", "Unknown")
        # Call update methods
        self.update_quick_stats()
        self.update_performance_charts()
        self.load_generation_history()
        # Optionally update tools, logs, etc.
