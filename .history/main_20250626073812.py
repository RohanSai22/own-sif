...
class PrometheusOrchestrator:
    ...
    def _initialize_population(self):
        ...
        self.tui.log_action("Population", f"Found {len(existing_agents)} existing agents for seeding", "INFO")
        
        self.population = []
        for i in range(config.population_size):
            if existing_agents and i < len(existing_agents):
    ...
    def run_evolution_loop(self):
        ...
        # Verify critical methods exist before starting evolution
        critical_methods = ["_evaluate_population", "_initialize_population", "_run_evaluation_phase"]
        missing_methods = [method for method in critical_methods if not hasattr(self, method)]
        
        if missing_methods:
            error_msg = f"Critical methods missing: {missing_methods}"
            logger.error(error_msg)
            self.tui.log_action("System", error_msg, "ERROR")
            return
    ...
    def _run_evaluation_phase(self) -> List[Any]:
        ...
        self.tui.log_thought(f"Running batch evaluation for agent {self.current_agent.agent_id}")
        ...
        try:
            ...
            self.tui.log_action("Evaluation", f"📝 Evaluating {batch_size} tasks starting from index {start_index}", "INFO")
            ...
            self.tui.log_action("Evaluation", f"✅ Completed {len(results)} evaluations", "SUCCESS")
            
            # Log summary
            summary = self.evaluator.get_evaluation_summary(results)
            self.tui.log_action(
                "Evaluation", 
                f"📊 Results: {summary['success_rate']:.1%} success, {summary['average_score']:.3f} avg score",
                "SUCCESS"
            )
            
            # Show individual results in thought log
            for result in results[:3]:  # Show first 3 results
                status = "✅" if result.success else "❌"
                self.tui.log_thought(f"{status} {result.instance_id}: {result.score:.3f}")
        ...
        try:
            ...
            try:
                ...
                if "proposed_changes" in improvement_data and improvement_data["proposed_changes"]:
                    ...
                    # Critical methods that should never be modified
                    critical_methods = [
                        "_evaluate_population", "_initialize_population", "_run_evaluation_phase",
                        "run_evolution_loop", "_select_parents", "_crossover_and_mutate",
                        "__init__", "_shutdown_evolution"
                    ]
                    ...
                    for change in improvement_data["proposed_changes"]:
                        ...
                        # Block changes to critical orchestrator methods
                        is_critical_change = False
                        if "main.py" in file_path:
                            old_code = change.get("old_code", "")
                            for critical_method in critical_methods:
                                if f"def {critical_method}" in old_code:
                                    is_critical_change = True
                                    self.tui.log_action("Safety", f"Blocked critical method modification: {critical_method}", "WARNING")
                                    break
                    ...
                    if safe_changes:
                        ...
                        if mutation_result["success"]:
                            # Verify critical methods still exist after mutation
                            critical_methods_intact = all(hasattr(self, method) for method in [
                                "_evaluate_population", "_initialize_population", "_run_evaluation_phase"
                            ])
                            
                            if not critical_methods_intact:
                                self.tui.log_action("Safety", "Critical methods damaged by mutation - reverting", "ERROR")
                                # Don't create new generation if critical methods were damaged
                            else:
                                self.tui.log_action(
                                    "Mutation",
    ...
    def _run_exploration_phase(self):
        """Run curiosity-driven exploration phase (Pillar 3)."""
        self.tui.update_status("Conducting curiosity-driven exploration...")
        self.tui.log_action("Exploration", "Starting autonomous exploration", "INFO")
        
        try:
            # Load current knowledge base
            knowledge_summary = self.current_agent._load_knowledge_base()
            
            # Conduct exploration
            self.tui.log_thought("Exploring new research directions and updating knowledge base...")
            exploration_results = self.current_agent._conduct_exploration(knowledge_summary)
            
            if exploration_results:
                # Parse exploration results
                try:
                    exploration_data = json.loads(exploration_results)
                    
                    # Update knowledge base with new insights
                    if "new_insights" in exploration_data and exploration_data["new_insights"]:
                        self.tui.log_action("Exploration", f"Discovered {len(exploration_data['new_insights'])} new insights", "SUCCESS")
                        
                        # Update knowledge base
                        success = self.current_agent._update_knowledge_base(exploration_data["new_insights"])
                        if success:
                            self.tui.log_action("Knowledge", "Knowledge base updated with exploration insights", "SUCCESS")
                        else:
                            self.tui.log_action("Knowledge", "Failed to update knowledge base", "WARNING")
                    
                    # Log exploration queries for future reference
                    if "exploration_queries" in exploration_data and exploration_data["exploration_queries"]:
                        self.tui.log_action("Exploration", f"Generated {len(exploration_data['exploration_queries'])} research questions", "INFO")
                        for query in exploration_data["exploration_queries"][:3]:  # Log top 3
                            self.tui.log_thought(f"Research question: {query}")
                    
                except json.JSONDecodeError as e:
                    logger.error(f"Failed to parse exploration results: {e}")
                    self.tui.log_action("Exploration", "Exploration parsing failed", "WARNING")
            else:
                self.tui.log_action("Exploration", "No exploration results generated", "WARNING")
                
        except Exception as e:
            logger.error(f"Exploration phase failed: {e}")
            self.tui.log_action("Exploration", f"Exploration failed: {e}", "ERROR")
    ...
    def _evaluate_population(self) -> List[List[Any]]:
        """Evaluate all agents in the population in parallel."""
        self.tui.update_status("Evaluating population...")
        self.tui.log_action("Evaluation", f"Evaluating population of {len(self.population)} agents", "INFO")
        ...
        for i, agent in enumerate(self.population):
            ...
            self.tui.log_action("Evaluation", f"🧪 Evaluating agent {i+1}/{len(self.population)}: {agent.agent_id}", "INFO")
            self.tui.log_thought(f"Testing agent performance on SWE-bench tasks...")
            
            # Set current agent for compatibility with existing evaluation code
            ...
            # Calculate and store score for this agent
            agent_score = self._calculate_agent_score(evaluation_results)
            if i >= len(self.population_scores):
                self.population_scores.extend([0.0] * (i + 1 - len(self.population_scores)))
            self.population_scores[i] = agent_score
            
            self.tui.log_action("Evaluation", f"📊 Agent {agent.agent_id}: Score = {agent_score:.3f}", "SUCCESS")
            
            # Update best score
            if agent_score > self.best_score:
                self.best_score = agent_score
                self.tui.log_action("Evolution", f"New best score: {agent_score:.3f} (Agent {agent.agent_id})", "SUCCESS")
        
        return all_evaluation_results
    
    def _select_parents(self) -> List[PrometheusAgent]:
        """Select parent agents using tournament selection."""
        tournament_size = max(2, len(self.population) // 4)
        selected_parents = []
        
        for _ in range(config.population_size):
            # Tournament selection
            tournament_indices = random.sample(range(len(self.population)), min(tournament_size, len(self.population)))
            tournament_scores = [self.population_scores[i] for i in tournament_indices]
    ...
    def _crossover_and_mutate(self, parents: List[PrometheusAgent]) -> List[PrometheusAgent]:
        """Create next generation through crossover and mutation with elitism."""
        next_generation = []
        
        # Elitism: Keep top 20% of current population
        elite_count = max(1, config.population_size // 5)
        sorted_indices = sorted(range(len(self.population_scores)), key=lambda i: self.population_scores[i], reverse=True)
        
        for i in range(elite_count):
            elite_agent = self.population[sorted_indices[i]].create_child_agent({"mutations_applied": []})
            elite_agent.generation = self.generation + 1
        ...
        while len(next_generation) < config.population_size:
            ...
            # Simple crossover: use one parent as base and apply mutations
            base_parent = parent1 if random.random() < 0.5 else parent2
            
            # Create offspring through self-improvement
            try:
                # Run self-improvement on the parent to create offspring
                source_code = base_parent.get_source_code()
                performance_logs = f"Parent performance: {self.population_scores[self.population.index(base_parent)]:.3f}"
                
                improvement_json = base_parent.self_reflect_and_improve(source_code, performance_logs)
                
                # Apply mutations to create offspring
                mutation_result = self.mutator.apply_patch(improvement_json)
                
                if mutation_result["success"]:
                    offspring = base_parent.create_child_agent(mutation_result)
                    offspring.generation = self.generation + 1
                    offspring.parent_id = base_parent.agent_id
                    next_generation.append(offspring)
                else:
                    # If mutation fails, create a simple copy with minor changes
                    offspring = base_parent.create_child_agent({"mutations_applied": []})
                    offspring.generation = self.generation + 1
                    offspring.parent_id = base_parent.agent_id
                    next_generation.append(offspring)
                    
            except Exception as e:
                logger.warning(f"Crossover/mutation failed for {base_parent.agent_id}: {e}")
    ...
    def _archive_population(self, all_evaluation_results: List[List[Any]]):
        """Archive the entire current population."""
        try:
            for i, (agent, evaluation_results) in enumerate(zip(self.population, all_evaluation_results)):
                score = self.population_scores[i] if i < len(self.population_scores) else 0.0
                ...
                self.archive.archive_agent(agent, metadata=metadata)
                
        except Exception as e:
            logger.error(f"Failed to archive population: {e}")
            self.tui.log_action("Archive", f"Population archival failed: {e}", "ERROR")
